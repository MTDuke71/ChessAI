...
int main() {
    ...
    return 0;
            ...
            std::cout << "Castling move found:\n";
            std::cout << "  From: " << from << " (" << indexToAlgebraic(from) << ")\n";
            std::cout << "  To: " << to << " (" << indexToAlgebraic(to) << ")\n";
            std::cout << "  Difference: " << (to - from) << "\n";
            std::cout << "  Decoded: " << decoded << "\n";
            std::cout << "  Legal: " << (board.isMoveLegal(decoded) ? "YES" : "NO") << "\n\n";
        }
    }
    
    // Also test the king moves that aren't castling
    std::cout << "=== TESTING KING MOVES ===\n";
    for (auto move : moves) {
        int from = moveFrom(move);
        if (from == 4) { // King on e1
            int special = moveSpecial(move);
            std::string decoded = decodeMove(move);
            std::cout << "King move: " << decoded << " (special=" << special << ")\n";
        }
    }
