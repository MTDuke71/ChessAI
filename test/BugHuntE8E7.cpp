...
int main() {
    ...
    std::cout << "=== BUG ANALYSIS: e8-e7 Position ===" << std::endl;
    std::cout << "Position: 4k2R/4K3/8/8/8/8/8/8 w - - 2 2" << std::endl;
    std::cout << "This position shows a 14-node discrepancy (Expected: 12331, Getting less)" << std::endl;
    std::cout << std::endl;
    ...
    if (totalDelta != 0) {
        ...
        std::cout << "The engine is missing " << (-totalDelta) << " nodes in this position." << std::endl;
        std::cout << "Need to investigate moves with non-zero deltas." << std::endl;
        
        // Identify problematic moves
        std::cout << std::endl;
        std::cout << "Problematic moves:" << std::endl;
        for (const auto& move : moves) {
            std::string moveStr = decodeMove(move);
            std::string key = moveStr;
            if (key.find('-') != std::string::npos) {
            ...
            if (delta != 0) {
                std::cout << "- " << moveStr << ": " << delta << " nodes difference" << std::endl;
            }
        }
    }
